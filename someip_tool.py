...
def _iface_matches(candidate: str, target: str) -> bool:
    """Return True when a scapy/interface entry matches the requested iface.

    Some Linux stacks expose VLAN subinterfaces with an @-suffix (e.g.,
    "eth1.12@eth1"). Accept either the full name or the portion before "@"
    so users can pass the shorter variant seen in `ip a` output.
    """
...
def _collect_ipv6_from_ip_cmd(iface: str) -> list[str]:
    """Collect IPv6 addresses from `ip -6 addr show dev <iface>` output.

    This is a last-resort fallback for systems where Scapy metadata does not
    expose IPv6 addresses for VLAN or special interfaces.
    """

    try:
        output = subprocess.check_output(
            ["ip", "-6", "addr", "show", "dev", iface], text=True
        )
    except Exception:
        return []
    ...
    for line in output.splitlines():
        ...
        if line.startswith("inet6"):
            ...
            if len(parts) >= 2:
                ...
                addr_only = addr_with_prefix.split("/", 1)[0]
                results.append(addr_only)
    return results


def resolve_ipv6_address(iface: str) -> str:
    addresses = []
    # Primary source: in6_getifaddr from Scapy (works across 2.7 variants)
    for entry in in6_getifaddr():
        name, addr = _extract_in6_entry(entry)
        if _iface_matches(name, iface) and addr:
            addresses.append(str(addr))

    # Fallback: Scapy interface metadata (ip6 attribute)
    iface_meta = None
    if hasattr(conf, "ifaces"):
        try:
            iface_meta = conf.ifaces.dev_from_name(iface)
        except Exception:
            iface_meta = None
    if iface_meta:
        ip6_val = getattr(iface_meta, "ip6", None)
        if ip6_val:
            addresses.append(str(ip6_val))
        ip6_list = getattr(iface_meta, "ip6s", None)
        if ip6_list:
            addresses.extend([str(x) for x in ip6_list if x])

    # Fallback: direct getter from scapy.arch
    if not addresses:
        try:
            addr_val = get_if_addr6(iface)
            if addr_val:
                addresses.append(str(addr_val))
        except Exception:
            pass

    # Last resort: call out to `ip -6 addr` for interfaces Scapy cannot parse
    if not addresses:
        addresses.extend(_collect_ipv6_from_ip_cmd(iface))
